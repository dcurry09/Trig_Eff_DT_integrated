...
private:
  edm::InputTag L1extraTag;
    ...
    std::vector<int>* cscsegs_sector;
    std::vector<int>* cscsegs_endcap;
    std::vector<int>* cscsegs_station;
    std::vector<int>* cscsegs_ring;
    std::vector<int>* cscsegs_chamber;
    std::vector<int>* cscsegs_wheel;
    
    std::vector<int>* dtsegs_station;
    std::vector<int>* dtsegs_sector;
    std::vector<int>* dtsegs_chamber;
    std::vector<int>* dtsegs_wheel;

    std::vector<float>* dtsegs_gbl_x;
    std::vector<float>* dtsegs_gbl_y;
    std::vector<float>* dtsegs_gbl_z;
    std::vector<float>* dtsegs_gbl_eta;
    std::vector<float>* dtsegs_gbl_theta;
    std::vector<float>* dtsegs_gbl_phi;
    ...
    int muon_dtsegs_station[MAX_MUONS][MAX_SEGS_STD];
    int muon_cscsegs_wheel[MAX_MUONS][MAX_SEGS_STD];
    int muon_cscsegs_sector[MAX_MUONS][MAX_SEGS_STD];
    int muon_dtsegs_chamber[MAX_MUONS][MAX_SEGS_STD];
    
    float muon_dtsegs_gbl_eta[MAX_MUONS][MAX_SEGS_STD];
    float muon_dtsegs_gbl_phi[MAX_MUONS][MAX_SEGS_STD];
    
    // isLCTAble
    int muon_cscsegs_islctable[MAX_MUONS][MAX_SEGS_STD];
    int muon_cscsegs_ismatched[MAX_MUONS][MAX_SEGS_STD];
    int muon_dtsegs_ismatched[MAX_MUONS][MAX_SEGS_STD];    // lctId is the position of the lct in the all LCT collection
    int muon_isdtseg[MAX_MUONS][MAX_SEGS_STD];
    
    // look also in fillAllLCTs
    int muon_cscsegs_lctId[MAX_MUONS][MAX_SEGS_STD];
    int muon_dtsegs_lctId[MAX_MUONS][MAX_SEGS_STD];
    ...
    class dt_Segment {
    public:
      const DTRecSegment4D dtsegcand;
      int MatchedHits;
      dt_Segment( const DTRecSegment4D &dt_seg, int MHits):dtsegcand(dt_seg), MatchedHits(MHits){};
    };
    
    typedef std::vector< Segment* > SegmentVector;
    typedef std::vector< dt_Segment* > dt_SegmentVector;
    ...
    TrigEff::dt_SegmentVector* dt_SegmentsInMuon(const reco::Muon* muon, const DTRecSegment4DCollection* dt_segments);
    ...
    std::vector<int>* dt_NumLCTsTrk;
    
    int dt_trLctEndcap[MAX_CSCTF_TRK][MAX_LCTS_PER_TRK]; 
    int dt_trLctSector[MAX_CSCTF_TRK][MAX_LCTS_PER_TRK]; 
    int dt_trLctSubSector[MAX_CSCTF_TRK][MAX_LCTS_PER_TRK]; 
    int dt_trLctBx[MAX_CSCTF_TRK][MAX_LCTS_PER_TRK]; 
    int dt_trLctBx0[MAX_CSCTF_TRK][MAX_LCTS_PER_TRK]; 
    int dt_trLctStation[MAX_CSCTF_TRK][MAX_LCTS_PER_TRK]; 
    int dt_trLctRing[MAX_CSCTF_TRK][MAX_LCTS_PER_TRK]; 
    int dt_trLctChamber[MAX_CSCTF_TRK][MAX_LCTS_PER_TRK]; 
    int dt_trLctTriggerCSCID[MAX_CSCTF_TRK][MAX_LCTS_PER_TRK]; 
    int dt_trLctFpga[MAX_CSCTF_TRK][MAX_LCTS_PER_TRK];	  
    int dt_trLctlocalPhi[MAX_CSCTF_TRK][MAX_LCTS_PER_TRK]; 
    int dt_trLctglobalPhi[MAX_CSCTF_TRK][MAX_LCTS_PER_TRK];   
    int dt_trLctglobalEta[MAX_CSCTF_TRK][MAX_LCTS_PER_TRK]; 
    int dt_trLctstripNum[MAX_CSCTF_TRK][MAX_LCTS_PER_TRK];   
    int dt_trLctwireGroup[MAX_CSCTF_TRK][MAX_LCTS_PER_TRK];     
    ...
    // ALL LCTS
    void fillAllLCTs(const edm::Handle< vector<L1TMuon::TriggerPrimitive> > lcts,
		     CSCSectorReceiverLUT* srLUTs_[5][2]
		     );
    ...
    std::vector<double>* trig_prim_phi;
    std::vector<double>* trig_prim_eta;

    int dt_SizeLCTs;
    std::vector<int>* dt_lctEndcap;
    std::vector<int>* dt_lctSector;
    std::vector<int>* dt_lctSubSector;
    std::vector<int>* dt_lctBx;
    std::vector<int>* dt_lctBx0;
    std::vector<int>* dt_lctStation;
    std::vector<int>* dt_lctRing;
    std::vector<int>* dt_lctChamber;
    std::vector<int>* dt_lctTriggerCSCID;
    std::vector<int>* dt_lctFpga;
    std::vector<float>* dt_lctlocalPhi;
    std::vector<float>* dt_lctglobalPhi;
    std::vector<float>* dt_lctglobalEta;
    std::vector<int>* dt_lctstripNum;
    std::vector<int>* dt_lctwireGroup;
