...
TrigEff::TrigEff(const edm::ParameterSet& pset):edm::EDAnalyzer(),
  ...
  recoMuons->Branch("rchEtaMatrixLocal"   , rchRingMatrix   , "rchRingMatrix[muonSize][35]/I");
  ...
  //-------------------------coMuons->Branch("muonNsegs",&muonNsegs);
...
void TrigEff::csctfDel() {	
  ...
  vector<int>().swap(*lctTheta_bti);


}

void TrigEff::fillCSCTF(const edm::Handle< vector<L1TMuon::InternalTrack> > tracks,
			//const edm::Handle< vector<L1TMuon::InternalTrack> > dt_tracks,
  ...
  for( auto trk = tracks->cbegin(); trk < tracks->cend(); trk++){
    ...
    */
    //cout << " trEndcap = " << trEndcap << endl; 
    //cout << " trSEctor = " << trSector << endl;
    // fill the track vectors
    EndcapTrk -> push_back(trEndcap);
...
void TrigEff::fillAllLCTs(const edm::Handle< vector<L1TMuon::TriggerPrimitive> > lcts,
  ...
  for ( auto Lct = lcts->cbegin(); Lct != lcts->cend(); Lct++) {
    ...
    if ( Lct->subsystem() == 1 ) {
      ...
      //unsigned lct_sector        = Lct->getGlobalSector();
      //unsigned lct_subsector     = Lct->getSubSector();
	...
	//cout <<"lctRing         = " << (*corrLct).first.ring()    << std::endl;
    ...
    if ( Lct->subsystem() == 0 ) {
      ...
      int dt_lct_bx        = Lct->getDTData().bx;
      int dt_lct_sector    = Lct->getDTData().sector;
      int dt_lct_wheel         = Lct->getDTData().wheel;
      int dt_lct_phi_bit        = Lct->getDTData().radialAngle;
      int dt_lct_bend           = Lct->getDTData().bendingAngle;
      int dt_lct_theta_bti      = Lct->getDTData().theta_bti_group;
      int dt_lct_theta_code     = Lct->getDTData().theta_code;
	...
	cout << "  Phi Bit(after convert)    = "       <<  dt_to_csc_phi_bit << endl;
	cout << "  Bunch X   = "  << Lct->getDTData().bx    << endl;
	cout << "  Sector    = "  << dt_lct_sector     << endl;
	cout << "  Station   = "  << dt_lct_station    << endl;
	cout << "  Wheel     = "  << dt_lct_wheel      << endl;
	cout << "  Bend      = "  << dt_lct_bend       << endl;
	cout << "  Theta_bti = "  << dt_lct_theta_bti  << endl;
	cout << "  Theta_code= "  << dt_lct_theta_code << endl;
      }
      ...
      lctTheta_bti ->push_back(dt_lct_theta_bti);
